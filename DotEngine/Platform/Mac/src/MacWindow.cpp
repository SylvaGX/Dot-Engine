...
namespace DotEngine {
    ...
    void Platform::Init(ApplicationContext& app) {
        ...
        Backend::Metal::Init(ctx.graphics, glfwWindow);
        ...
        glfwSetWindowCloseCallback(glfwWindow, [](GLFWwindow* w) {
            ...
            DotData::JsonSerializer::Save(application->engine.window, DotData::Paths::EngineData() / "WindowConfig.json", 1);
        ...
        glfwSetKeyCallback(glfwWindow, [](GLFWwindow* w, int key, int, int action, int) {
            auto* application = GetApp(w);
            if (!application)
                return;
        ...
        glfwSetMouseButtonCallback(glfwWindow, [](GLFWwindow* w, int button, int action, int) {
            auto* application = GetApp(w);
            if (!application)
                return;
    ...
    void Platform::Present(EngineContext& /*ctx*/) {
        // Metal presentation is done inside Editor::End via command buffer commit
    }

    void Platform::Shutdown(EngineContext& ctx) {
        auto* window = static_cast<GLFWwindow*>(ctx.window.nativeHandle);
        if (window) {
            Backend::Metal::Shutdown(ctx.graphics);
